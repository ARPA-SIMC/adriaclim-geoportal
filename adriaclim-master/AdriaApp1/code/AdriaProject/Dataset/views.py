...
def getDataPolygonNew(request):
    ...
    is_indicator = request.data.get('isIndicator')
    # print("IS INDICATOR:",is_indicator)
    dataVect=allFunctions.getDataPolygonNew(dataset_id,layer_name,date_start,date_end,lat_lng_obj,statistic,time_op,num_param,range,is_indicator,lat_min,lat_max,lng_min,lng_max,parametro_agg)
    return JsonResponse({'dataVect':dataVect})